<?php
/* SVN FILE: $Id: Qrbg.php 252 2009-11-29 08:28:51Z david@ramaboo.com $ */
/**
		...
		if (BOO_QRBG_CACHE && $count == 1) { // cache is only supported for single requests
			...
			try {
				...
				if (!$result) {
					...
					$cmd = BOO_QRBG_CMD . " --user=\"{$this->username}\" --pass=\"{$this->password}\" --count=\"" 
						. BOO_QRBG_CACHE_SIZE . "\" --type=\"{$type}\" --text";
					exec($cmd, $cache);
